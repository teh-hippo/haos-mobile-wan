...
async def test_hassio_step_updates_existing_discovered_entry_when_uuid_matches(hass) -> None:
    ...
    other_entry = MockConfigEntry(
        ...
        data={CONF_URL: "http://other-gateway.local:8099", CONF_TOKEN: "other-token"},
    )
...
async def test_hassio_step_rejects_uuid_rediscovery_when_url_matches_distinct_gateway(
    hass,
) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
    ...
    other_entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id="other-gateway-uuid",
        data={CONF_URL: "http://gateway.local:8099", CONF_TOKEN: "other-token"},
    )
    other_entry.add_to_hass(hass)
    ...
    assert result["reason"] == "single_instance_allowed"
    assert entry.data == {
        CONF_URL: "http://old-gateway.local:8099",
